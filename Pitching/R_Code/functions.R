...
{
  data2 <- data %>% 
    left_join(teamnames, by = "Team") %>% 
    select(-K_pct, -BB_pct) %>% 
    mutate(Strike_pct = round((Strikes / Pitches) * 100,1),
           Season = as.integer(Season),
           K_pct = round(SO / TBF, 3) * 100,
           BB_pct = round(BB / TBF, 3) * 100) %>% 
    select(playerid, Season, Name, Team, League, Age, G, GS, IP, Start_IP, Relief_IP, 
           TBF, Pitches, ERA, tERA, FIP, xFIP, WAR, AVG, WHIP, BABIP, IBB, HBP, SO, K_9, 
           K_pct, BB, BB_9, BB_pct, K_BB, H, H_9, HR, HR_9, LD_pct, GB_pct, FB_pct, 
           HR_FB, Strike_pct, FBv, FBall_pct, `O-Swing_pct`, `Z-Swing_pct`, Swing_pct, 
           `O-Contact_pct`, `Z-Contact_pct`, Contact_pct, Zone_pct, `F-Strike_pct`, 
           SwStr_pct, GB, FB, LD, `ERA-`, `FIP-`, `xFIP-`) %>%
    rename("xFIP_minus" = "xFIP-", "FIP_minus" = "FIP-", "ERA_minus" = "ERA-",
           "O_Swing_pct" = "O-Swing_pct", "Z_Swing_pct" = "Z-Swing_pct",
           "O_Contact_pct" = "O-Contact_pct", "Z_Contact_pct" = "Z-Contact_pct", 
           "F_Strike_pct" = "F-Strike_pct") %>% 
    tidyr::replace_na(list(Start_IP = 0, Relief_IP = 0, FBall_pct = 0)) %>% 
    mutate(Pos_Group = case_when(Start_IP > Relief_IP ~ 'SP',
                                 Start_IP <= Relief_IP ~ 'RP'),
           G_pct = case_when(Season == 2020 ~ round(G / 60, 3),
                                    TRUE ~ round(G / 162, 3)),
...
add_projection_years <- function(data, forecast_year) 
{
  ...
  data <- data %>% 
    ...
    arrange(Name, Season) %>% 
    group_by(playerid) %>% 
    tidyr::fill(Pos_Group, .direction = 'downup') %>% 
    filter(Season >= 2002) 
  return(data)
}
...
# Merge FanGraphs pitching stats to projected, current, prior, 2 prior, and 3 prior seasons
# Add position and create position groups for current season, extrapolate position groups
# for all current seasons
merge_pitching_stats <- function(data, pitching_stats_list)
{
  pitching_info <- data %>% select(Name, Team, Season_Current, Age_Current, Playerid, 
  ...
  pitching_stats <- data %>% select(-Age_Current, -MLB_Service_Current, -MLB_Service_Projected, 
  ...
  data2 <- pitching_info %>%
    left_join(pitching_stats[, c("Playerid","Season","Pitches","Pos_Group",pitching_stats_list)], 
              by = c("Playerid","Season_Current" = "Season")) %>% 
    rename("Pos_Group_Current" = "Pos_Group", "Pitches_Current" = "Pitches")
  colnames(data2)[(ncol(data2) - length(pitching_stats_list) + 1):ncol(data2)] <- paste0(colnames(data2[, pitching_stats_list]), "_Current")
  
  data2 <- data2 %>%
    left_join(pitching_stats[, c("Playerid","Season",pitching_stats_list)], 
              by = c("Playerid","Season_Prior" = "Season"))
  colnames(data2)[(ncol(data2) - length(pitching_stats_list) + 1):ncol(data2)] <- paste0(colnames(data2[, pitching_stats_list]), "_Prior")
  
  data2 <- data2 %>%
    left_join(pitching_stats[, c("Playerid","Season",pitching_stats_list)], 
              by = c("Playerid","Season_Prior_2" = "Season"))
  colnames(data2)[(ncol(data2) - length(pitching_stats_list) + 1):ncol(data2)] <- paste0(colnames(data2[, pitching_stats_list]), "_Prior_2")
  
  data2 <- data2 %>%
    left_join(pitching_stats[, c("Playerid","Season",pitching_stats_list)], 
              by = c("Playerid","Season_Prior_3" = "Season"))
  colnames(data2)[(ncol(data2) - length(pitching_stats_list) + 1):ncol(data2)] <- paste0(colnames(data2[, pitching_stats_list]), "_Prior_3")
  
  data2 <- data2 %>%
    left_join(pitching_stats[, c("Playerid","Season","Pitches","Pos_Group",pitching_stats_list)], 
              by = c("Playerid","Season_Projected" = "Season")) %>% 
    rename("Pos_Group_Projected" = "Pos_Group", "Pitches_Projected" = "Pitches")
  colnames(data2)[(ncol(data2) - length(pitching_stats_list) + 1):ncol(data2)] <- paste0(colnames(data2[, pitching_stats_list]), "_Projected")
...
train_models <- function(historical_data, y_var, x_vars, model_type, tuneLength, years_out, plot = T)
{
  historical_years_out <- historical_data %>% 
    ...
    tidyr::drop_na() %>% mutate(Pos_Group_Current = as.factor(Pos_Group_Current))
  ...
  if (sum(grepl("Pos_Group_Current" , colnames(future2))) > 0)
  {
    ...
    future2 <- future2 %>% mutate(Pos_Group_Current = as.factor(Pos_Group_Current))
  }
  ...
  if (y_var %in%  c("G_pct_Projected","GS_pct_Projected"))
  {
  ...
  if (model_type == "qrf")
  {
    model <- caret::train(as.formula(paste0(y_var, " ~ ", paste0(x_vars2, collapse = " + "))),
                          ...
                          weights = Num_Pitches_Harmonic, trControl = control, metric = 'MAE')
  ...
  if (model_type == "qrf")
  {
    ...
    preds <- data.frame(predict(model$finalModel, newdata = newdata, 
